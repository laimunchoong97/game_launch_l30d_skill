...
Produce a complete, evidence-led sense-check for an upcoming or recently released game. The report is for public-data market assessment and inventory posture, not an exact forecast of a specific reseller's key sales.
...
Use Standard by default. Use Quick for a list of many games. Use Deep for a title with meaningful upfront exposure or conflicting evidence.
...
Read `references/report-template.md` and use it in full. Do not replace it with a short summary unless the user explicitly requests Quick mode. Include:
...
Use `scripts/steam_public.py` for keyless public Steam facts. Use `scripts/score_release.py` for reproducible weighted scoring when a normalized signal JSON is available. Run `python -m unittest discover -s tests` before publishing changes to this skill.
